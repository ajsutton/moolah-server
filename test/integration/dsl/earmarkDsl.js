...
export default class EarmarksDsl {
  ...
  async verifyEarmark(args) {
    ...
    const response = await this.server.get(
      `/api/earmarks/${encodeURIComponent(earmark.id)}/`,
      options.statusCode
    );
    if (response.statusCode === 200) {
      assert.deepEqual(
        ...
        earmark,
        `Did not match earmark ${options.alias}`
      );
    }
  }
  ...
  async verifyEarmarks(args) {
    ...
    if (options.earmarks !== undefined) {
      ...
      const expectedEarmarks = options.earmarks.map(alias =>
        this.getEarmark(alias)
      );
      assert.includeDeepMembers(
        actualEarmarks,
        expectedEarmarks,
        'Did not find all expected earmarks.'
      );
    }
  }
  ...
  async modifyEarmark(args) {
    ...
    const response = await this.server.put(
      '/api/earmarks/' + encodeURIComponent(currentEarmark.id) + '/',
      modifiedEarmark,
      options.statusCode
    );
