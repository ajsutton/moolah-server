...
describe('Investment Value DAO', function () {
  ...
  it('should round trip values', async function () {
    ...
    const allValues = await valueDao.getValues(userId, {
      accountId: accountId,
    });
  ...
  it('should update existing value', async function () {
    ...
    assert.deepEqual(allValues, [{ date: '2023-02-21', value: 16000 }]);
  });
  ...
  it('should remove value', async function () {
    ...
    assert.deepEqual(allValues, [{ date: '2023-02-21', value: 15000 }]);
  });
});
