...
describe('Transaction DAO', function () {
  ...
  describe('Storing Transactions', function () {
    it('should round trip a transaction', async function () {
      ...
      assert.deepEqual(
        await transactionDao.transactions(userId, {
          accountId: 'account-id',
          scheduled: true,
        }),
        [transaction]
    ...
    it('should create transaction with minimal required values', async function () {
      ...
      const result = await transactionDao.transaction(
        userId,
    ...
    it('should return undefined when transaction belongs to a different user', async function () {
      await transactionDao.create(userId, minimalTransaction);
      const transaction = await transactionDao.transaction(
        'someone else',
        minimalTransaction.id
      );
      assert.isUndefined(transaction);
    ...
    it('should update transaction', async function () {
      ...
      assert.deepEqual(
        await transactionDao.transaction(userId, originalTransaction.id),
        modifiedTransaction
      );
    });
    ...
    it('should delete transaction', async function () {
      ...
      assert.isUndefined(
        await transactionDao.transaction(userId, minimalTransaction.id)
      );
    });
  });
  ...
  describe('Searching Transations', function () {
    ...
    it('should include transfer to the account in the list of transactions', async function () {
      ...
      const transaction2 = makeTransaction({
        ...
        type: 'transfer',
        toAccountId: minimalTransaction.accountId,
        amount: -2000,
        date: '2017-05-30',
      });
      const transaction3 = makeTransaction({ amount: 300, date: '2017-06-03' });
  ...
  it('should count transactions', async function () {
    ...
    const count = await transactionDao.transactionCount(userId, {
      accountId: minimalTransaction.accountId,
      scheduled: false,
    });
  ...
  describe('Balance Calculations', function () {
    ...
    it('should calculate account balance prior to transaction', async function () {
      ...
      const balance = await transactionDao.balance(
        userId,
        { accountId: minimalTransaction.accountId },
        transaction2
      );
  ...
  describe('Balance by Category', function () {
    ...
    beforeEach(async function () {
      ...
      await categoryDao.create(userId, {
        id: 'test2a',
        name: 'Test2a',
        parentId: 'test2',
      });
    ...
    it('should calculate total balance by category', async function () {
      await transactionDao.create(
        userId,
        makeTransaction({ amount: 5000, categoryId: 'test1' })
      );
      await transactionDao.create(
        userId,
        makeTransaction({ amount: -2000, categoryId: 'test1' })
      );
      await transactionDao.create(
        userId,
        makeTransaction({ amount: 300, categoryId: 'test2' })
      );
