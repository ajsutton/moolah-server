export default class InvestmentValueDao {
  ...
  async getLatestValue(userId, accountId) {
    const results = await this.query(
      'SELECT IFNULL(value, 0) as value FROM investment_value WHERE user_id = ? AND account_id = ? ORDER BY date DESC LIMIT 1',
      userId,
      accountId
    );
  ...
  async getValues(userId, options = {}) {
    ...
    let query =
      'SELECT date, value FROM investment_value WHERE user_id = ? AND account_id = ?';
    if (opts.from) {
      query += ' AND date >= ?';
      args.push(opts.from);
    }
  ...
  async removeValue(userId, accountId, date) {
    await this.query(
      'DELETE FROM investment_value WHERE user_id = ? AND account_id = ? AND date = ?',
      userId,
      accountId,
      date
    );
