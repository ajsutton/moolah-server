...
function extrapolateScheduledTransaction(transaction, forecastUntil) {
  forecastUntil =
    typeof forecastUntil == 'string' ? parseISO(forecastUntil) : forecastUntil;
  if (transaction.recurPeriod === 'ONCE') {
    return isAfter(parseISO(transaction.date), forecastUntil)
      ? []
      : [transaction];
  }
  const instances = [];
  ...
  while (!isAfter(date, forecastUntil)) {
    instances.push(
      Object.assign({}, transaction, { date: dateFormat(date, 'yyyy-MM-dd') })
    );
...
function extrapolateScheduledTransactions(
  scheduledTransactions,
  forecastUntil
) {
  return scheduledTransactions
    .map(transaction =>
      extrapolateScheduledTransaction(transaction, forecastUntil)
    )
...
export default {
  ...
  forecastBalances(
    ...
    const transactions = extrapolateScheduledTransactions(
    ...
    return Object.entries(balances).map(([date, data]) => ({
      date,
