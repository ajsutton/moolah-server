...
export default {
  ...
  handler: async function (request) {
    ...
    return await db.withTransaction(request, async daos => {
      ...
      let searchOptions;
      try {
        searchOptions = await transactionSearchOptions.parseOptions(
          request,
          daos
        );
      } catch (errorMessage) {
      ...
      const transactions = await daos.transactions.transactions(
        userId,
        searchOptionsWithPaging
      );
      const hasMore = transactions.length > pageSize;
      const priorBalance = hasMore
        ...
        : 0;
      const totalNumberOfTransactions =
        await daos.transactions.transactionCount(userId, searchOptions);
      return {
