...
async function isInvalidToAccountId(transactionData, daos, userId) {
  const toAccount = await daos.accounts.account(
    userId,
...
export default async function validateTransaction(transaction, daos, userId) {
  ...
  if (
    hasAccount &&
    (await daos.accounts.account(userId, transaction.accountId)) === undefined
  ) {
  ...
  if (
    !hasAccount &&
    (transaction.earmark === null || transaction.earmark === undefined)
  ) {
  ...
  if (
    ...
    (await isInvalidToAccountId(transaction, daos, userId))
  ) {
    return Boom.badRequest('Invalid toAccountId');
  }
  if (
    transaction.recurEvery !== undefined &&
    transaction.recurPeriod === undefined
  ) {
    return Boom.badRequest(
      'recurEvery is only applicable when recurPeriod is set'
    );
  }
  ...
  if (
    ...
    (transaction.toAccountId === undefined || transaction.toAccountId === null)
  ) {
  ...
  if (
    transaction.earmark !== undefined &&
    transaction.earmark !== null &&
    (await isInvalidEarmarkId(daos, userId, transaction.earmark))
  ) {
