...
export default {
  ...
  async parseOptions(request, daos) {
    ...
    if (
      accountId !== undefined &&
      (await daos.accounts.account(userId, accountId)) === undefined
    ) {
      throw 'Account not found';
    }
    if (
      earmarkId !== undefined &&
      (await daos.earmarks.earmark(userId, earmarkId)) === undefined
    ) {
    ...
    const missingCategories = loadedCategories.filter(
      data => data.category === undefined
    );
    if (missingCategories.length > 0) {
      throw (
        'Unknown category ' + missingCategories.map(data => data.id).join(', ')
      );
    }
    return {
      accountId,
